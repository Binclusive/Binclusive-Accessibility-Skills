...
description: Audit React or Next.js accessibility from a prior Binclusive project map. Use when the user says /auditaccessibility, audit accessibility, run accessibility audit, accessibility test, "binclusive projemi test et", "erişilebilirlik testi yap", "erişilebilirlik todo çıkar", "accessibility todo çıkar", or wants an accessibility TODO report from mapped routes, views, components, or paths.
---

# Audit Accessibility

Audit a previously mapped React/Next.js scope and write an actionable accessibility TODO report. This skill observes and documents only. It never edits source code.
...
5. Detect framework/language from the map. For React/Next.js, read:
