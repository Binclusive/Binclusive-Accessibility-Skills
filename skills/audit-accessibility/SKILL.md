...
- **Engine-groundable now — run the CLI:** `npx @binclusive/cli scan <path> --format json --from-skill audit-accessibility` runs the multi-collector static dispatch — one command grounds every platform below whose collector it dispatches. (`--from-skill` attributes the run to this skill so skill-driven usage is counted.)
  - **React / Next.js web** (static source).
  - **Astro** (static source).
  - **Shopify/Liquid** (static source).
  - **SwiftUI/UIKit** (static source; the Swift toolchain must be present).
  - **Jetpack Compose / Kotlin** (static source; the Kotlin/Gradle toolchain must be present).
  - **Android Views/XML** (static source).
  - **Unity** (static source, when the target is a Unity project).
  - **Any live page, any framework** (rendered DOM): `npx @binclusive/cli scan --url <url> --format json --from-skill audit-accessibility`.
...
1. **Run the engine first.** `npx @binclusive/cli scan <path> --format json --from-skill audit-accessibility` for static source — one run dispatches every engine-groundable collector (React/TSX, Astro, Shopify/Liquid, SwiftUI/UIKit, Compose/Kotlin, Android Views/XML, Unity), and/or `npx @binclusive/cli scan --url <url> --format json --from-skill audit-accessibility` for a rendered page. If the project has no `binclusive.json`, it is auto-detected; `npx @binclusive/cli init` writes one when you want it pinned.
...
For the no-engine platforms above, the audit stays exactly as it is today (agent + references); every such finding is `agent: judgment` or `agent: runtime`.
