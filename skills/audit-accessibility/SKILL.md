...
- **Engine-groundable now — run the CLI:** `npx @binclusive/cli scan <path> --format json` runs the multi-collector static dispatch — one command grounds every platform below whose collector it dispatches.
...
1. **Run the engine first.** `npx @binclusive/cli scan <path> --format json` for static source — one run dispatches every engine-groundable collector (React/TSX, Astro, Shopify/Liquid, SwiftUI/UIKit, Compose/Kotlin, Android Views/XML, Unity), and/or `npx @binclusive/cli scan --url <url> --format json` for a rendered page. If the project has no `binclusive.json`, it is auto-detected; `npx @binclusive/cli init` writes one when you want it pinned.
...
- Never report a runtime-dependent criterion as satisfied from static code alone. When the trigger condition is present — single-character shortcuts (WCAG 2.1.4, even when focus-scoped), custom composite widgets that must switch AT into forms/application mode, live-region announcement cadence — raise it as `RUNTIME-CHECK` (`agent: runtime`) with the specific hypothesis to verify in JAWS and NVDA, instead of clearing it or staying silent. A static pass can prove a violation exists; it can never prove one doesn't.
