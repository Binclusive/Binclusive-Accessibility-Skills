...
- **Modern semantic roles and state:** when supported by the project's pinned Flutter SDK, prefer `SemanticsRole`, `headingLevel`, `expanded`, `mixed`, `isRequired`, `validationResult`, `linkUrl`, `onExpand`, `onCollapse`, and `onDismiss` over encoding the same information into a label string. Check the SDK constraint before adding a newer API; do not silently raise the minimum Flutter version.
